...
export async function getDepartmentInfoById(
	user_access_token: string,
	...
	var config = {
		method: "GET",
		url:
			"https://open.feishu.cn/open-apis/contact/v3/departments/parent?department_id=" +
			department_id,
		headers: {
			Authorization: `Bearer ${access_token}`,
		},
	};
	const fatherData = await axios(config);
	var config2 = {
		method: "GET",
		url:
	...
	console.log(fatherData.data.data.items, data.data.data.department);
	if (fatherData.data.data.items?.length) {
		return [...fatherData.data.data.items, data.data.data.department];
	}
	return [data.data.data.department];
}
